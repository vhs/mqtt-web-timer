...
class TimerService {
    ...
    static stop () {
      debug('stop')
      return TimerService.intervalId != null ? clearInterval(TimerService.intervalId) : false
    }

    static getTimer () {
      debug('getTimer')
      return (TimerService.timer != null && TimerService.timer.time != null) ? TimerService.timer.time : false
    }

    static removeTimer () {
      debug('removeTimer')
      if (TimerService.timer != null && TimerService.timer.callback != null) TimerService.timer.callback()
    ...
    static runner () {
      if (TimerService.timer != null && TimerService.timer.time != null && TimerService.timer.time <= Date.now()) {
        debug('runner', 'TIME!')
